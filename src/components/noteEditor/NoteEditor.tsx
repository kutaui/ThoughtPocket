...
import {Note} from "@/app/(dash)/dashboard/page"
...
export default function NoteEditor({activeNote, fetchNotes, setActiveNote}: any) {
    ...
    if (!activeNote)
        return <div className="mx-auto mt-36 text-3xl text-gray-500">No Active Note</div>;
    ...
    return (
        <div className="flex flex-col w-full mt-10">
            ...
            <Tiptap value={updatedBody} onChange={handleBody} activeNote={activeNote}/>
            {savingState === SavingState.SAVING && <div className="ml-4 text-2xl">Saving...</div>}
            {savingState === SavingState.SAVED && <div className="ml-4 text-2xl">Note Saved</div>}
        </div>
    );
}